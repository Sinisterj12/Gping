    ...
    def __init__(self, tcping_path=None):
        ...
        if not os.path.exists(self.tcping_path):
            raise FileNotFoundError(
                f"Error: tcping.exe not found at {self.tcping_path}"
            )
...
class PingTool:
    ...
    def __init__(self, root):
        ...
        # Initialize system tray after GUI is ready
        self.root.after(100, self.setup_system_tray)
    ...
    def init_csv_log(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                "Cannot create log file. Please run as administrator or check folder permissions.",
            )
        except Exception as e:
    ...
    def calculate_packet_loss(self, ip_address):
        ...
        return round(
            (self.failed_pings[ip_address] / self.total_pings[ip_address]) * 100, 1
        )
    ...
    def log_to_csv(
        ...
        try:
            ...
            response_time_str = (
                f"{response_time:.1f}ms" if response_time is not None else ""
            )
            packet_loss = (
                f"{self.calculate_packet_loss(ip_address)}%"
                if not skip_packet_loss
                else ""
            )
            ...
            if (
                status == "RESTORED"
                or len(self._csv_buffer) >= 10
                or time.time() - self._last_flush > 5
            ):
    ...
    def minimize_to_tray(self):
        if self.is_running:
            ...
            # No notification as per user request
        else:
    ...
    def quit_app(self, icon, item):
        if self.is_running:
            self.stop_ping()  # Stop ping threads
    ...
    def check_network_profile(self):
        ...
        try:
            ...
            self.network_label.configure(
                text=f"Error checking network: {str(e)}", text_color="#FF0000"
            )
    ...
    def create_ip_frame(self):
        ...
        # Gateway IP row
        self.gateway_check = ctk.CTkCheckBox(
            gateway_frame, text="Gateway IP", command=self.update_start_button_state
        )
        self.gateway_check.pack(side="left", padx=(5, 0))

        ctk.CTkLabel(
            gateway_frame, text="Gateway IP:", font=ctk.CTkFont(size=12), width=80
        ...
        self.gateway_indicator = ctk.CTkLabel(
            gateway_frame,
            text="○",
            text_color="gray",
        ...
        self.gateway_status = ctk.CTkLabel(
            gateway_frame, text="Not Running", font=ctk.CTkFont(size=12), width=150
        )
        self.gateway_status.pack(side="right", padx=5)

        # Google DNS row (formerly LAN IP)
        self.dns_check = ctk.CTkCheckBox(
            lan_frame, text="Google DNS", command=self.update_start_button_state
        )
        self.dns_check.pack(side="left", padx=(5, 0))

        ctk.CTkLabel(
            lan_frame, text="Google DNS:", font=ctk.CTkFont(size=12), width=80
        ).pack(side="left", padx=5)
        ...
        ctk.CTkButton(
            lan_frame, text="Save", command=lambda: self.save_single_ip("lan"), width=60
        ).pack(side="left", padx=5)

        self.lan_indicator = ctk.CTkLabel(
            lan_frame,
            text="○",
            text_color="gray",
            font=ctk.CTkFont(size=16, weight="bold"),
        ...
        self.lan_status = ctk.CTkLabel(
            lan_frame, text="Not Running", font=ctk.CTkFont(size=12), width=150
        )
    ...
    def detect_ip_addresses(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                "Gateway detection timed out. Please try again or enter IP manually.",
            )
        except PermissionError:
            messagebox.showerror(
                "Error",
                "Access denied. Please run as administrator for automatic gateway detection.",
            )
        except Exception as e:
    ...
    def create_control_frame(self):
        ...
        self.start_button = ctk.CTkButton(
            control_frame, text="Start Tests", command=self.start_ping, state="disabled"
        )
    ...
    def create_results_frame(self):
        ...
        self.results_text = ctk.CTkTextbox(
            results_frame, wrap="word", font=ctk.CTkFont(family="Consolas", size=11)
        )
    ...
    def update_status_indicator(self, status, target):
        ...
        if (
            target in self._last_update
            and current_time - self._last_update[target] < 0.35
        ):
            return
        ...
        # Schedule the update
        self.root.after(
            10, self._apply_status_update, indicator, status_label, color, text, icon
        )
    ...
    def start_ping(self):
        ...
        if not self.is_running:
            ...
            # Reset counters
            self.total_pings = {gateway_ip: 0, google_dns: 0}
            self.failed_pings = {gateway_ip: 0, google_dns: 0}
            ...
            if self.gateway_check.get():
                ...
                self.gateway_ping_thread = threading.Thread(
                    target=self.ping, args=(gateway_ip, "gateway")
                )
            ...
            if self.dns_check.get():
                ...
                self.lan_ping_thread = threading.Thread(
                    target=self.ping, args=(google_dns, "google_dns")
                )
    ...
    def ping(self, ip_address, ping_type):
        ...
        while self.is_running:
            ...
            if is_up:
                ...
                if (
                    not self.ip_status[ip_address] and consecutive_successes >= 2
                ):  # Need 2 successful pings to confirm recovery
                    downtime = time.time() - self.down_start_times[ip_address]
                    downtime_str = self.format_duration(downtime)
                    self.log_event(
                        f"{ping_type.upper()}:     Connection restored to {ip_address} - Response: {ping_time:.1f}ms (Down for {downtime_str})"
                    )
                    self.log_to_csv(
                        ping_type.upper(),
                        ip_address,
                    ...
                    self.log_event(
                        f"{ping_type.upper()}:     Response from {ip_address} - Response: {ping_time:.1f}ms"
                    )
                    self.log_to_csv(
                        ping_type.upper(),
                        ip_address,
                ...
                if (
                    self.ip_status[ip_address] and consecutive_failures >= 3
                ):  # Need 3 failures to confirm down
                    self.ip_status[ip_address] = False
                    self.down_start_times[ip_address] = current_time
                    if not self.failure_logged[ip_address]:
                        self.log_event(
                            f"{ping_type.upper()}:     Connection LOST to {ip_address} at {datetime.now().strftime('%H:%M:%S')}"
                        )
                        self.log_to_csv(
                            ping_type.upper(),
                            ip_address,
    ...
    def setup_system_tray(self):
        ...
        try:
            icon_path = os.path.join(self.base_path, "Gping.ico")
            if os.path.exists(icon_path):
                image = Image.open(icon_path)
            else:
                image = Image.new("RGB", (64, 64), color="blue")

            self.tray_icon = pystray.Icon("GPing", image, "GPing Network Monitor", menu=pystray.Menu(
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = subprocess.run(
            check_cmd, capture_output=True, text=True, startupinfo=startupinfo
        )
